...
The Xmodem that has been implemented here is the original specification with one small change. It uses 128-byte packets, and an 8-bit CRC check. The original spec (which came from CP/M) uses char[26] to pad out files to reach the 128-byte packet size. In our implementation, we use char[0] instead (which also appears to be what other Commodore programs like Color64 and CCGMS do). Xmodem-CRC will likely work with this implementation (as Xmodem-CRC was made to be backwards compatible), but Xmodem-1K will likely not work. I would love to also implement Punter Protocol, but my understanding is it is much harder to implment than Xmodem (thus I took the easier path!).
